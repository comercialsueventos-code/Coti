...
export class MachineryService {
   ...
   */
  static async getAll(filters?: MachineryFilters): Promise<Machinery[]> {
    let query = supabase
      .from('machinery')
      .select('*')
      .order('category, name')
   ...
   * Get machinery by ID
   */
  static async getById(id: number): Promise<Machinery> {
   ...
   */
  static async create(machineryData: CreateMachineryData): Promise<Machinery> {
    const processedData = {
      ...machineryData,
      requires_operator: machineryData.requires_operator ?? false,
      maintenance_cost_per_use: machineryData.maintenance_cost_per_use ?? 0,
      fuel_cost_per_hour: machineryData.fuel_cost_per_hour ?? 0,
      insurance_required: machineryData.insurance_required ?? false,
      is_available: true,
      is_active: true
    }
   ...
   * Update machinery
   */
  static async update(id: number, machineryData: UpdateMachineryData): Promise<Machinery> {
    const { data, error } = await supabase
      .from('machinery')
   ...
   */
  static async getByCategory(category: string): Promise<Machinery[]> {
    const { data, error } = await supabase
      .from('machinery')
      .select('*')
   ...
   */
  static async getAvailableMachinery(
    date: string,
    hours: number,
    category?: string
  ): Promise<Machinery[]> {
  ...
  static async getStatistics(): Promise<{
    ...
    return {
      total_machinery: machinery?.length || 0,
   ...
   * Search machinery
   */
  static async search(searchTerm: string): Promise<Machinery[]> {
    const { data, error } = await supabase
      .from('machinery')
      .select('*')
   ...
   */
  static async markUnavailable(id: number, reason: string): Promise<Machinery> {
   ...
   */
  static async markAvailable(id: number): Promise<Machinery> {
    return await this.update(id, {
      is_available: true
    })
  }
   ...
   */
  static async getMachineryNeedingMaintenance(): Promise<Machinery[]> {
